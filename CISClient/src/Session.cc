...
void Session::Init()
{
	...
	string strService = m_appConfigPtr->ReadOne(CfgTradeList("GetTradeListSCName"));
	add_in_bus(in_bus);
	YSUserBusAddString(in_bus, YSDICT_SERVNAME, strService.c_str(), strService.length());

	m_func(in_bus, &out_bus);

	void* appBin = YSUserBusGet(out_bus, m_appConfigPtr->ReadOne(CfgTradeList("Output")).c_str());
	if (!appBin)
	...
	for (int i = 0; i < hash_base; ++i) {
		...
		for (int j = 0; j < link_len; ++j) {
            ...
            string ver = GetStringFromStruct(link_obj, m_appConfigPtr->ReadOne(CfgTradeList("ServiceInfo/version")));
			string app = GetStringFromStruct(link_obj, m_appConfigPtr->ReadOne(CfgTradeList("ServiceInfo/application")));
			string fun = GetStringFromStruct(link_obj, m_appConfigPtr->ReadOne(CfgTradeList("ServiceInfo/function")));
			string lib = GetStringFromStruct(link_obj, m_appConfigPtr->ReadOne(CfgTradeList("ServiceInfo/library")));
			string dic_ver = GetStringFromStruct(link_obj, m_appConfigPtr->ReadOne(CfgTradeList("ServiceInfo/dictory_ver")));
			string desc_info = Glib::convert(GetStringFromStruct(link_obj, m_appConfigPtr->ReadOne(CfgTradeList("ServiceInfo/descript"))),
				"utf-8", "gb2312");

			StringArray inputArr;
			StringArray outputArr;

			string name = GetStringFromStruct(link_obj, m_appConfigPtr->ReadOne(CfgTradeList("ServiceInfo/name")));
