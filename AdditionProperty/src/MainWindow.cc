...
MainWindow::MainWindow() :
    m_infoFramePtr(), m_serviceList(), m_btnSendRequest(), m_vBox(false, 5),
            m_hBox1()
{
    m_appConfigPtr.reset(new AppConfig());

    InitServerInfo();
    m_sessionPtr.reset(new Session(m_appConfigPtr, sigc::mem_fun(*this,
            &MainWindow::ServiceCallSock)));
    if (!m_sessionPtr)
    ...
    m_btnSendRequest.set_label(m_appConfigPtr->ReadOne(CfgMainWindow(
            "SendRequest")));
    m_btnAddArgument.set_label(m_appConfigPtr->ReadOne(CfgMainWindow(
            "AddArgument")));
...
void
MainWindow::on_service_list_select_change()
{
    ...
    {
        Gtk::MessageDialog infoDlg(msg, false, Gtk::MESSAGE_INFO,
                Gtk::BUTTONS_OK, true);
        infoDlg.run();
    }
}
...
void
MainWindow::output_var_to_xml_file(void* var, const string& sep = "========")
{
    void* str = NULL;
    while (1)
    {
...
void
MainWindow::add_argument(string& arg)
{
...
MainWindow::on_send_button_clicked()
{
    ...
    try
    {
        ...
        string service_name =
                m_sessionPtr->get_service_info(service_desc_info).name;

        YSUserBusAddString(send_bus, YSDICT_SERVNAME, service_name.c_str(),
                service_name.length());
        ...
        if (!recv_bus)
            throw string(
                    "Error occurred when recv data, create recvBus failed!");
...
MainWindow::InitServiceList()
{
    try
    {
        ...
        int cols_count = m_serviceList.append_column(m_appConfigPtr->ReadOne(
                CfgMainWindow("TradeListColumnName")), m_columns.item);
        ...
        for (std::list<string>::iterator it = service_list.begin(); it
                != service_list.end(); ++it)
        {
            Gtk::TreeRow row = *(m_refTreeStore->append());
            string tmp = *it;
            row.set_value(m_columns.item, tmp);
            Session::ServiceInfo& serviceInfo = m_sessionPtr->get_service_info(
                    *it);
...
void
MainWindow::YsArrayToStringArray(void* var_arr, StringArray& str_arr)
{
...
void
MainWindow::ServiceCallSock(void* p_in_bus, void** pp_out_bus)
{
    char* ip = m_ip;
    short port = m_port;

    if (FALSE == YSServiceClientCallSock(ip, port, m_time_out, p_in_bus,
            pp_out_bus))
    {
#ifdef __OS_WIN__
...
void
MainWindow::UpdateOutputFrame(StringArray &output_args, void* recv_bus)
{
    for (StringArray::iterator it = output_args.begin(); it
            != output_args.end(); ++it)
    {
...
void
MainWindow::UpdateInputFrame(StringArray &input_args)
{
}
