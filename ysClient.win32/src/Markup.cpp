...
{
	// Insert strInsert int str at nLeft replacing nReplace chars
...
int TextEncoding::PerformConversion( void* pTo, MCD_CSTR pszToEncoding/*=NULL*/ )
{
	...
	else if ( nToCP == MCD_UTF32  ) // to UTF-32 from UTF-8/ANSI
	{
		...
		else // from ANSI
		{
			...
			// So converting via UTF-16LE, though not sure why UTF-16LE works instead of BE
...
bool TextEncoding::FindRaggedEnd( int& nTruncBeforeBytes )
{
	...
	else // UTF-8, SBCS DBCS
	{
		if ( nCP == MCD_UTF8 )
		{
			...
			nTruncBeforeBytes = pUTF8End - pUTF8Last;
		}
		else
		{
...
struct ElemStack
{
	...
	void Unslot( TagPos& lp ) { int n=lp.iSlotNext,p=lp.iSlotPrev; if (n) pL[n].iSlotNext=p; if (p) pL[p].iSlotPrev=n; else anTable[lp.nSlot]=n; };
...
struct FilePos
{
	FilePos()
	{
		...
		m_nFileCharUnitSize=0; m_nOpFileTextLen=0; m_pstrBuffer=NULL; m_nReadBufferStart=0; m_nReadBufferRemoved=0;
	};
	...
	bool FileFlush( int nWriteStrLen = -1, bool bFflush = false );
	bool FileClose();
...
bool FilePos::FileWriteText( const MCD_STR& strDoc, int nWriteStrLen/*=-1*/ )
{
	...
	else // single or multibyte file (i.e. not UTF-16)
	{
		...
		textencoding.PerformConversion( (void*)pMBBuffer );
		x_AddResult( m_strIOResult, MCD_T("converted_from"), MCD_T("UTF-16"), MRC_ENCODING|MRC_LENGTH, nWriteStrLen );
		if ( textencoding.m_nFailedChars )
			x_AddResult( m_strIOResult, MCD_T("conversion_loss") );
...
bool FilePos::FileFlush( int nWriteStrLen/*=-1*/, bool bFflush/*=false*/ )
{
	bool bSuccess = true;
	MCD_STRCLEAR( m_strIOResult );
	MCD_STR& strBuffer = *m_pstrBuffer;
	if ( nWriteStrLen == -1 )
...
void CMarkup::x_SetDebugState()
{
	...
	if ( m_nNodeLength || m_nNodeOffset || (m_pFilePos && ! m_pFilePos->FileAtTop()) )
	{
		if ( ! m_nNodeLength )
...
MCD_STR CMarkup::x_GetPath( int iPos ) const
{
	...
	{
		if ( iPos )
			iPos = m_pFilePos->m_elemstack.iTop;
	}
	while ( iPos )
	{
		MCD_STR strTagName;
...
bool CMarkup::x_SetAttrib( int iPos, MCD_PCSZ pAttrib, MCD_PCSZ pValue, int nFlags /*=0*/ )
{
	...
	if ( m_nDocFlags & MDF_WRITEFILE )
	{
		...
		if ( nNodeStart && nNewDocLength > m_pFilePos->m_nBlockSizeBasis )
		{
			...
			{
				m_pFilePos->FileFlush( nNodeStart );
