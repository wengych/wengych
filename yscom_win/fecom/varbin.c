...
BOOL  FEVarBinSetLen(void *Var,INT32 Len)
{
    ...
    if ( FEVARBIN_MEM_S(Var)>FEVARBIN_MEM_L(Var) )
    {
        memset(((BYTE*)FEVARBIN_MEM_V(Var))+FEVARBIN_MEM_L(Var) \
            ,0,FEVARBIN_MEM_S(Var)-FEVARBIN_MEM_L(Var));
    }
    return TRUE;
}
...
BOOL  FEVarBinMalloc(void *Var,INT32 L)
{
    ...
    if ( (0>=L) || (FEVARBIN_MEM_L(Var)+L<FEVARBIN_MEM_S(Var)) )
    {
        return TRUE;
    }
...
BOOL  FEVarBinMalloc2(void *Var,INT32 L)
{
    ...
    if ( (0>=L) || (L<FEVARBIN_MEM_S(Var)) )
    {
        return TRUE;
    }
    return FEVarBinRealloc(Var,L-FEVARBIN_MEM_S(Var));
}
