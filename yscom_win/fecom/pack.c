...
{
    BYTE *vv;
    if ( (INT32)(P+BYTE_SIZE)>FEVarBinGetLen(Buf) )
    {
        return FALSE;
    }
...
{
    BYTE *vv;
    if ( (INT32)(P+INT16_SIZE)>FEVarBinGetLen(Buf) )
    {
        return FALSE;
    }
    if ( NULL==(vv=(BYTE*)FEVarBinGet(Buf)) )
...
{
    BYTE *vv;
    if ( (INT32)(P+INT32_SIZE)>FEVarBinGetLen(Buf) )
    {
        return FALSE;
    }
    if ( NULL==(vv=(BYTE*)FEVarBinGet(Buf)) )
...
{
    BYTE *vv;
    if ( (INT32)(P+INT64_SIZE)>FEVarBinGetLen(Buf) )
    {
        return FALSE;
    }
    if ( NULL==(vv=(BYTE*)FEVarBinGet(Buf)) )
...
BOOL  FEUnPackFuncDouble(void *Buf,INT32 P,DOUBLE *v,BYTE *DP)
{
    ...
    if ( (INT32)(P+BYTE_SIZE+DOUBLE_SIZE)>FEVarBinGetLen(Buf) )
    {
        return FALSE;
    }
    if ( NULL==(vv=(BYTE*)FEVarBinGet(Buf)) )
